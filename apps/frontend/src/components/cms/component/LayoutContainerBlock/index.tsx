...
import {
  ...
  type LayoutContainerBlockDataFragment,
} from "@/gql/graphql";
import {
  columnClassMap,
  gapSizeClassMap,
 ...
 */
export const LayoutContainerBlockComponent: CmsComponent<
  Partial<LayoutContainerBlockDataFragment>
> = ({ data, children }) => {
  ...
  if (columnClassMap[columns ?? 1])
    innerClasses.push(columnClassMap[columns ?? 1]);
  if (color && colorMap[color]) additionalClasses.push(colorMap[color]);
  if (paddingTop && paddingTopMap[paddingTop])
    additionalClasses.push(paddingTopMap[paddingTop]);
  if (paddingBottom && paddingBottomMap[paddingBottom])
    additionalClasses.push(paddingBottomMap[paddingBottom]);
  if (marginTop && marginTopMap[marginTop])
    additionalClasses.push(marginTopMap[marginTop]);
  if (marginBottom && marginBottomMap[marginBottom])
    additionalClasses.push(marginBottomMap[marginBottom]);
  ...
  return (
    ...
    >
      <div
        className={`container mx-auto place-items-center grid ${innerClasses.join(
          " "
        )}`}
      >
        {children
...
LayoutContainerBlockComponent.displayName =
  "Layout (Component/LayoutContainerBlock)";
LayoutContainerBlockComponent.getDataFragment = () => [
